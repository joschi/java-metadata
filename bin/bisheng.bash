...
download_file 'https://marketplace-api.adoptium.net/v1/info/available_releases/huawei' "${RELEASES_FILE}"
AVAILABLE_RELEASES=$(jq '.available_releases[]' "${RELEASES_FILE}")

for release in ${AVAILABLE_RELEASES}
do
	page=0
	while download_file "https://marketplace-api.adoptium.net/v1/assets/feature_releases/huawei/${release}?page=${page}&page_size=20&sort_order=ASC" "${TEMP_DIR}/release-${release}-${page}.json"
	do
		page=$((page+1))
	done
done
...
jq -s -S . "${METADATA_DIR}"/bisheng*.json > "${METADATA_DIR}/all.json"
